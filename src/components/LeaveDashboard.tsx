...
import { ChevronLeft, ChevronRight, AlertTriangle, X, Users, FolderHeart, CalendarCheck, ShieldAlert } from "lucide-react";
...
export default function LeaveDashboard({ formateurs, disciplines, conges, isDark = false }: LeaveDashboardProps) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 bg-white dark:bg-slate-900/40 border border-slate-200/50 dark:border-white/[0.03] p-5 rounded-2xl shadow-sm mb-6 transition-all duration-500">
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center bg-slate-50 dark:bg-slate-950 border border-slate-200/60 dark:border-white/[0.04] p-1.5 rounded-xl transition-all duration-300">
            <button onClick={prevMonth} className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-850 rounded-lg transition-colors text-slate-650 dark:text-slate-400 cursor-pointer">
              <ChevronLeft className="w-4 h-4"/>
            </button>
            <div className="flex-1 text-center text-sm font-bold capitalize text-slate-800 dark:text-slate-200 select-none">
              {format(currentDate, "MMMM yyyy", { locale: fr })}
            </div>
            <button onClick={nextMonth} className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-850 rounded-lg transition-colors text-slate-650 dark:text-slate-400 cursor-pointer">
            ...
            >
              ...
              <div className={cn(
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-black tracking-tight text-slate-800 dark:text-slate-100">DÉTAILS DU CONGÉ</h3>
                    <p className="text-[10px] uppercase font-bold text-slate-450 dark:text-slate-500 tracking-wider mt-0.5">Fiche d'indisponibilité</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <div className="font-extrabold text-base text-slate-850 dark:text-slate-150 flex items-center gap-2">
                ...
                {selectedConge.commentaire ? (
                  <div>
                    ...
                    <p className="text-xs text-slate-600 dark:text-slate-350 bg-slate-50 dark:bg-slate-950/20 border border-slate-100 dark:border-white/[0.01] p-3.5 rounded-2xl whitespace-pre-wrap leading-relaxed">
                      "{selectedConge.commentaire}"
                    </p>
                  </div>
                ) : (
              ...
              {/* Close Button Footer */}
              <div className="p-4 bg-slate-50/70 dark:bg-slate-950/20 border-t border-slate-100 dark:border-white/[0.03] flex justify-end">
                <button 
                  onClick={() => setSelectedConge(null)}
